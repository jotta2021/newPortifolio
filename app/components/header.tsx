  ...
  return (
    <div className={"flex w-[100vw] fixed top-0 z-50"} style={{ backgroundColor: darkMode ? 'black' : 'white' }}>
      <div className="flex justify-between my-3 mx-5 w-full items-center">
        <span className={darkMode ? 'text-white max-sm:hidden' : 'text-muted-foreground max-sm:hidden'}>
          Joanderson Luan <span className="text-cyan-500">{'</>'}</span>
        </span>
        <ul className="flex gap-4 items-center">
          <li className='cursor-pointer hover:text-cyan-500' onClick={() => scroll('about')}>{about}</li>
          <li className='cursor-pointer hover:text-cyan-500' onClick={() => scroll('projects')}>{projects}</li>
          <li className='cursor-pointer hover:text-cyan-500' onClick={() => scroll('skills')}>{Habilidades}</li>
          ...
          <Button variant='outline' size='icon' onClick={changeLanguage}>
            <Image src={language === 'pt-br' ? '/square.png' : '/eua.png'} width={30} height={30} alt='language' />
          </Button>
        </ul>
      </div>
    </div>
